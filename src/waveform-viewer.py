...
class WaveformViewer(object):
    ...
    # MARK: this code is relevant for data acquisition
    def init_audio(self):
        """ 
        ...
        self.CHUNK = 2048  # TODO: test if 2048 cause buffer overflow
    ...
    def start(self):
        """ Starts the Qt application needed for the waveform viewer to appear on screen """
        if (sys.flags.interactive != 1) or not hasattr(QtCore, 'PYQT_VERSION'):
            QtGui.QApplication.instance().exec_()

    # MARK: this code is relevant for converting byte data to floating point
    def callback(self):
        """
    ...
    def anim(self):
        """
        Creates a QTimer to call the callback function for updating the plot,
        then starts the timer and waveform viewer.
        """
        timer = QtCore.QTimer()
