...
\begin{itemize}
    ...
    \item Computer: 13-inch, 2012 MacBook Pro laptop with a \SI{2.5}{\giga \hertz} Intel Core i5 processor running macOS 10.14.6. Audio was captured using the laptop's built-in \SI{44100}{\hertz}, 2-channel microphone, but only one channel was used as filter input. Further microphone specifications are unfortunately not shown in the computer's hardware report.

    \item I used version 19 of the \texttt{PortAudio} library \cite{portaudio}, which is an audio I/O library for \texttt{C/C++}, to interface with the computer's microphone and sound card.

    \item I wrote the filter program itself in \texttt{Python}, using the \texttt{PyAudio} library \cite{pyaudio} to provide \texttt{Python} bindings for \texttt{PortAudio}. I used the \texttt{NumPy} library \cite{numpy} for efficient implementation of the fast Fourier transform, convolution, and common mathematical functions.

\end{itemize}
Audio data is captured from the microphone using \texttt{PortAudio} with an input stream with the specifications shown in Table \ref{tab:stream}. The number of frames per buffer is chosen so that the fast Fourier transform used to convolve the impulse response with the audio buffer has 1024 elements. The initialization of the audio stream is shown in Listing \ref{code:stream}.
...
\begin{table}[htb!]
    ...
    \begin{tabular}{|c|c|}
        ...
        Channels & 1\\
        Frames per buffer & $ 1024 - \texttt{len(h)} + 1 $\\
        \hline
    \end{tabular}
...
\noindent Data is read from the audio stream as a hexadecimal \texttt{Python} byte string,\footnote{Which has its peculiarities compared to \texttt{C/C++}, for example prefixing hexadecimal numbers with \texttt{\textbackslash x} instead of with \texttt{0x}.} which is then converted to an array of 16-bit signed integers with \texttt{NumPy}'s \texttt{frombuffer} function. For orientation, a representative audio stream output buffer is shown in Listing \ref{code:output}. The real-time filtering process works by convolving each buffer of audio data with the filter's impulse response using the overlap-add method \cite{convolution}, as shown in Listing \ref{code:filter}.
...
CHUNK = 1024 - len(h) + 1  # frames per buffer
pa = pyaudio.PyAudio()
stream = pa.open(format=pyaudio.paInt16,  # 16-bit signed integer
...
b'a\x13\xbd\x13\x9c\x13\xfe\x12\xed\x11]\x10P\x0e\xd0\x0b\xca\x08Z\x05'
>>> buffer_int16 = np.frombuffer(data, dtype=np.int16)
[4961 5053 5020 4862 4589 4189 3664 3024 2250 1370]
\end{minted}   
    \vspace{-1ex}
    \caption{Representative audio stream output as a byte string and 16-bit integer array.}
    \label{code:output}
\end{listing}
...
def get_filtered_buffer(buffer_in):
    ...
    :param buffer_in: 1D Numpy array of length CHUNK and datatype 
                       np.int16 holding raw audio buffer
...
\end{minted}
    \vspace{-1ex}
    \caption{The function \texttt{get\_filtered\_buffer} implements the overlap-add method used for real-time filtering. Complete code for the online filtering component of the project is in the \texttt{realtime.py} file.}
    \label{code:filter}
\end{listing}


\section{Representative Filtered Signals}
The following few figures were generated offline, all with sample rate $ f_{\mathrm{s}} = \SI{44100}{\hertz} $, and are meant to demonstrate the filter's desired phase shift and bandpass properties in a static situation. The real-time, online component of the project will be demonstrated in person.

\begin{figure}[htb!]
	\centering
	\includegraphics[width=0.85\linewidth]{test-time-shift}
    \vspace{-3ex}
	\caption{Demonstration of the filter's phase-shifting property with a \SI{1500}{\hertz} sinusoid. The dotted curve shows a copy of the input signal shifted by $ \pi/2 $ for comparison with the filtered output. Note that the filter's time domain properties are not ideal---the filtered signal's amplitude (red line) fluctuates somewhat over the first few periods shown here. (The amplitude does reach a steady state after about 5 periods, but this is not obvious from the four periods shown here.)}
	\label{fig:phase}
\end{figure}
